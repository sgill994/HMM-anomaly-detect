...
training_start <- as.Date("29/11/2009", format = "%d/%m/%Y")
training_end <- as.Date("21/11/2010", format = "%d/%m/%Y")
...
n_times <- replicate(51,nrow(training_data)/51)
n_states <- 8

# Train SM3 response variable as dependent on GAP due to 0.57 correlation between variables
model <- depmix(response = list(GAP~1, GRP~1, SM3~GAP),
                data = training_data,
                nstates = n_states,
                ntimes = n_times,
